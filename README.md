# MPP Solar Home Assistant Add-on v2.1.0
...
### v2.1.0 - Stable partial-response operation
- Downgrade successful partial-response fallback logging from warning to info
- Keep 5s updates and latest-payload selection behavior
- Package the communication fixes into the 2.1 release

### v2.0.14 - Latest partial payload selection
- When fallback parsing is used, parse only the latest partial QPIGS payload in the buffer
- Prevent stale partial replies from accumulating into 43/64/85+ value payloads
- Keep 5s publishing while improving data freshness
...
- Reassemble fragmented inverter frames instead of discarding them
- Keep the 5s scheduling improvements from v2.0.10/v2.0.11 while improving publish reliability

### v2.0.11 - Improved frame completion
- Waits for a complete `(payload)CRC\\r` frame before parsing
- No longer stops reading early just because a partial `\\r` arrived
- Slightly larger read budget while still keeping a fixed 5s cycle
...
- Metadata remains synchronized across add-on files
